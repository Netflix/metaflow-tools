...
resource "aws_iam_role" "iam_s3_access_role" {
  name = "${var.resource_prefix}-iam-role-${var.resource_suffix}-${data.aws_region.current.name}"
...
resource "aws_iam_policy" "kms_s3" {
  name        = "${var.resource_prefix}-kms-s3-${var.resource_suffix}-${data.aws_region.current.name}"
...
resource "aws_iam_policy" "s3_access" {
  name        = "${var.resource_prefix}-s3-access-${var.resource_suffix}-${data.aws_region.current.name}"
