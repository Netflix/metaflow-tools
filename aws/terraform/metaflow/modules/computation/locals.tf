locals {
  ...
  # Name of Batch Queue.
  batch_queue_name = "${var.resource_prefix}-${var.resource_suffix}"

  # Name of IAM role to create to manage ECS tasks
  ecs_execution_role_name = "${var.resource_prefix}-ecs-execution-role-${var.resource_suffix}-${data.aws_region.current.name}"

  # Name of Batch service IAM role
  batch_service_role_name = "${var.resource_prefix}-batch-service-iam-role-${var.resource_suffix}-${data.aws_region.current.name}"

  # Name of ECS IAM role
  ecs_instance_role_name = "${var.resource_prefix}-ecs-iam-role-${var.resource_suffix}-${data.aws_region.current.name}"
}
