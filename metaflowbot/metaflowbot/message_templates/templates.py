...
class RESPONSES(Enum):
    NO_FLOW_IN_NAMESPACE = (
        f"I couldn't find any flows on a Global namespace."
    ...
    USER_NOT_PRESENT = (
        ":meow_thinkingcool: It seems the User you may have been "
...
class Template:
    def _make_null_response(
        ...
        message = [
            ...
            {"type": "section", "text": {"type": "mrkdwn", "text": f"{response_line}"}},
        ]
...
class BotVersion(Template):
    ...
    def get_slack_message(self):
        message = (
            f"Running Metaflowbot version `{__version__}` with Metaflow version "
...
class IntroMessage(Template):
    def get_slack_message(self, bot_name):
        intro_message = (
            f"Hey, I am {bot_name}, the Metaflowbot :robot_face:! "
